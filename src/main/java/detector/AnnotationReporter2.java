...
public class AnnotationReporter2 {
	...
	/**
	 * store number of tickets with status="resolved"
	 */
	static int status_resolved_count = 0;
	/**
	...
	public static void createExcel() {
		...
		try {
			...
			/**
			 * Conditional Formatting for status="Resolved"
			 */
			String compare_status = "Resolved";
			HSSFConditionalFormattingRule cfrole_status_resolved = matchedConditionalFormatting
					.createConditionalFormattingRule(ComparisonOperator.EQUAL,
							"\"" + compare_status + "\"");

			HSSFPatternFormatting match_fill_pattern_resolved = cfrole_status_resolved
					.createPatternFormatting();
			match_fill_pattern_resolved
			...
			CellRangeAddress[] matched_data_range_status = { (CellRangeAddress) CellRangeAddress
					.valueOf("C1:C100") };
			matchedConditionalFormatting.addConditionalFormatting(
					matched_data_range_status, cfrole_status_resolved);
			/**
			 * Conditional Formatting for Fixed Resolution
			 */
			/*
			 * String compareItem = "Fixed"; HSSFConditionalFormattingRule
			 * cfrole_resolution_fixed = matchedConditionalFormatting
			 * .createConditionalFormattingRule(ComparisonOperator.EQUAL, "\"" +
			 * compareItem + "\"");
			 * 
			 * HSSFPatternFormatting match_fill_pattern_fixed =
			 * cfrole_resolution_fixed .createPatternFormatting();
			 * match_fill_pattern_fixed
			 * .setFillBackgroundColor(IndexedColors.GREEN.index);
			 * 
			 * CellRangeAddress[] matched_data_range_resolution= {
			 * (CellRangeAddress) CellRangeAddress .valueOf("D1:D100") };
			 * matchedConditionalFormatting.addConditionalFormatting(
			 * matched_data_range_resolution, cfrole_resolution_fixed);
			 */
			/**
			 * auto column size for matched ticket sheet
			 */
			...
			matchticketRowCount++;
			/**
			 * Print Number of Tickets with Status="Resolved"
			 */
			HSSFRow statusResolvedCountRow = matchedTicketSheet
					.createRow(matchticketRowCount);
			...
			statusResolvedCountRow.createCell(0).setCellValue(
					"Tickets Resolved");
			statusResolvedCountRow.createCell(1).setCellValue(
					status_resolved_count);
			matchticketRowCount++;
			/**
			 * Print Number of Tickets with Resolution="Fixed"
			 */
			/*
			 * HSSFRow ticketfixedrow = matchedTicketSheet
			 * .createRow(matchticketRowCount);
			 * 
			 * CellRangeAddress[] matched_data_range_fixed = {
			 * (CellRangeAddress) CellRangeAddress .valueOf("A" +
			 * (matchticketRowCount + 1) + ":B" + (matchticketRowCount + 1)) };
			 * matchedConditionalFormatting.addConditionalFormatting(
			 * matched_data_range_fixed, matchedCFrole3);
			 * ticketfixedrow.createCell(0).setCellValue("Tickets Fixed");
			 * ticketfixedrow.createCell(1).setCellValue(fixed_count);
			 * matchticketRowCount++;
			 */
			/**
			...
			jiraticketrow.createCell(1).setCellValue(ticketList.size());
			matchticketRowCount++;
			/**
		...
		public void reportMethodAnnotation(
			...
			try {
				...
				if (otherJiraTicket.find() == false
					...
					addTickets(matcherJiraTicket.group().toUpperCase(),
	...
	public static void matchedTickets() {
		...
		while (it.hasNext()) {
			...
			try {
				...
				if (classNamesList != null) {
					...
					try {
						if (status == null) {
							System.out.print("  Status-> null");
							resolutionValue = "null";
						} else if (status.getName().equals("Resolved")) {
							System.out.print("  Status-> Resolved");
						...
						System.out.print("Exception in status");
					}
					/**
		...
		/**
		 * display Tickets which are Fixed
		 */
		System.out.println("Number of Tickets with Status Resolved: "
				+ status_resolved_count);
		/**
		 * display Tickets which are Fixed
		 */
		 ...
		 */
		System.out.println("\nTotal Jira Found :" + ticketList.size());
	}
	...
	public static void createJson() {
		...
		try {
			JsonGenerator jsonGenerator = jasonFactory.createJsonGenerator(
					new File("src/main/java/detector/Annotation.json"),
					JsonEncoding.UTF8);
			Iterator<String> matchedTicketIterator = ticketList.keySet()
					.iterator();
			ArrayList<String> classNamesList = null;
			ArrayList<String> ticketParametersList = null;
			System.out.println("Matched Tickets JSON generating....");
			jsonGenerator.writeStartObject();
			jsonGenerator.writeFieldName("Matched_Tickets");
			...
			while (matchedTicketIterator.hasNext()) {
				...
				if (classNamesList != null) {
					// Ticket No
					jsonGenerator.writeStringField("Ticket_No", ticket);
					StringBuffer className = new StringBuffer();
					for (String value : classNamesList) {
						className.append(value + ", ");
					}
					...
					// classNames
					jsonGenerator
							.writeStringField("ClassNames", tempClassNames);
				}
				ticketParametersList = ticketParameters.get(ticket);
				if (ticketParametersList != null) {
					StringBuffer statusResolution = new StringBuffer();
					...
					String[] statusResolutionparts = statusResolution
							.toString().split(",");
					String status = statusResolutionparts[0];
					// Status
					...
					String resolution = statusResolutionparts[1];
					// Resolution
					jsonGenerator.writeStringField("Resolution", resolution);
					String fixVersion = statusResolutionparts[2];
					// Fix Version
					jsonGenerator.writeStringField("FixVersion", fixVersion);
